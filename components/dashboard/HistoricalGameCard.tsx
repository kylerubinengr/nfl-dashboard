...
const BettingResultDisplay = ({ bettingResult, homeAbbr, awayAbbr, homeScore, awayScore }: { bettingResult?: BettingResult, homeAbbr: string, awayAbbr: string, homeScore?: number, awayScore?: number }) => {
  ...
  return (
    <div className="flex justify-around items-center text-center p-3 bg-slate-50/70 border-t border-slate-100">
      <div className="flex items-center gap-2">
         <span className="text-[10px] uppercase font-bold text-slate-400 tracking-tight">Spread</span>
         <span className={`font-bold text-xs ${spreadBadgeColor}`}>
            {bettingResult.spreadCoveredBy} {bettingResult.spreadResult}
         </span>
      </div>
      <div className="h-4 w-px bg-slate-300" />
      <div className="flex items-center gap-2">
         <span className="text-[10px] uppercase font-bold text-slate-400 tracking-tight">Total</span>
         <span className={`font-bold text-xs ${totalBadgeColor}`}>
...
export function HistoricalGameCard({ game }: HistoricalGameCardProps) {
  ...
  return (
    <Link href={`/game/${game.id}`} className="block bg-white/80 backdrop-blur-sm border border-slate-200 rounded-lg shadow-lg hover:border-blue-500 hover:scale-[1.01] transition-all duration-300 overflow-hidden">
      <header className="px-4 py-3 flex justify-between items-start gap-4 bg-slate-50/50 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
            <span className="font-black text-slate-900 bg-slate-200 px-2 py-0.5 rounded text-[10px] uppercase tracking-widest">
                FINAL
            </span>
        </div>
      </header>
